...
BprimeRecoHists::BaseHists BprimeRecoHists::book_BaseHists(const std::string & name, const std::string & label, double minMass, double maxMass, double minPt, double maxPt){
  ...
  hists.mass = book<TH1F>("mass_"+name,"Mass "+label,100,minMass,maxMass);
  return hists;
}
...
BprimeRecoHists::BprimeRecoHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  chiDis = book<TH1F>("chiVal","#Chi^{2} Value",100, 0,150);
  ttbar_chi =  book<TH1F>("ttbarChi","#Chi^{2} Value for ttbar",100, 0,200);

  wHad_res_pt   = book<TH1F>("wHad_res_pt","W_{had} Resolution p_{T}", 100, -8, 2);
  wHad_res_E    = book<TH1F>("wHad_res_E","W_{had} Resolution E", 100, -8, 2); 
  wHad_res_mass = book<TH1F>("wHad_res_mass","W_{had} Resolution Mass", 100, -8, 2); 
  wHad_res_phi  = book<TH1F>("wHad_res_phi","W_{had} Resolution #phi", 100, 0, 3.14); 
  wHad_res_eta  = book<TH1F>(" wHad_res_eta","W_{had} Resolution #eta", 100, 0, 10); 
  wHad_res_deltaR  = book<TH1F>(" wHad_res_deltaR","W_{had} Resolution #Delta R", 100, 0, 10); 
  wLep_res_pt   = book<TH1F>("wLep_res_pt","W_{lep} Resolution p_{T}", 100, -8, 2);
  wLep_res_E    = book<TH1F>("wLep_res_E","W_{lep} Resolution E", 100, -8, 2); 
  wLep_res_mass = book<TH1F>("wLep_res_mass","W_{lep} Resolution Mass", 100, -8, 2); 
  wLep_res_phi  = book<TH1F>("wLep_res_phi","W_{lep} Resolution #phi", 100, 0, 3.14); 
  wLep_res_eta  = book<TH1F>("wLep_res_eta","W_{lep} Resolution #eta", 100, 0., 10); 
  wLep_res_deltaR  = book<TH1F>("wLep_res_deltaR","W_{lep} Resolution #Delta R", 100, 0., 10); 

  topHad_res_pt   = book<TH1F>("topHad_res_pt","Top_{had} Resolution p_{T}", 100, -8, 2);
  topHad_res_E    = book<TH1F>("topHad_res_E","Top_{had} Resolution E", 100, -8, 2); 
  topHad_res_mass = book<TH1F>("topHad_res_mass","Top_{had} Resolution Mass", 100, -8, 2); 
  topHad_res_phi  = book<TH1F>("topHad_res_phi","Top_{had} Resolution #phi", 100, 0, 3.14); 
  topHad_res_eta  = book<TH1F>("topHad_res_eta","Top_{had} Resolution #eta", 100, 0, 10); 
  topHad_res_deltaR  = book<TH1F>("topHad_res_deltaR","Top_{had} Resolution #Delta R", 100, 0, 10); 
  topLep_res_pt   = book<TH1F>("topLep_res_pt","Top_{lep} Resolution p_{T}", 100, -8, 2);
  topLep_res_E    = book<TH1F>("topLep_res_E","Top_{lep} Resolution E", 100, -8, 2); 
  topLep_res_mass = book<TH1F>("topLep_res_mass","Top_{lep} Resolution Mass", 100, -8, 2); 
...
void BprimeRecoHists::fill(const uhh2::Event & event){
  ...
  for(auto hyp :  event.get(hyps)){
    ...
    double ttbar = fabs((topJets+wlep).M()-174)*fabs((topJets+wlep).M()-174)/18/18+fabs(whad.M()-181)*fabs(whad.M()-181)/15/15;
    if(ttbar < ttbarchi)ttbarchi=ttbar;
  
    double tophad_chi = fabs((topJets+whad).M()-181)*fabs((topJets+whad).M()-181)/(15*15);//+fabs(wlep.M()-80.4)*fabs(wlep.M()-80.4)/(2*2);
    double toplep_chi = fabs((topJets+wlep).M()-174)*fabs((topJets+wlep).M()-174)/(18*18)+fabs(whad.M()-82)*fabs(whad.M()-80.4)/(9*9);
  ...
  //cout<<"reco "<<deltaR(whad_best,wlep_best)<<" gen "<<deltaR(GenInfo.get_wHad(),GenInfo.get_wLep())<<" diff "<<abs(deltaR(whad_best,wlep_best)-deltaR(GenInfo.get_wHad(),GenInfo.get_wLep())) <<endl;
  //if(wlep_best.pt()<200) return;
  fill_BaseHists(whad_best, wHad_best, weight);
  fill_BaseHists(wlep_best, wLep_best, weight);
  if(fabs((topJets_best+whad_best).M()-181)*fabs((topJets_best+whad_best).M()-181)/15/15<fabs((topJets_best+wlep_best).M()-175)*fabs((topJets_best+wlep_best).M()-175)/18/18){
    ...
    //if(GenInfo.get_topHad().pt()>0)cout<<"hadronic Top "<<deltaR(topJets_best+whad_best,GenInfo.get_topHad())<<endl;
  }
  else{
    fill_BaseHists(topJets_best+wlep_best,topLep_best,weight);
    //if(GenInfo.get_topLep().pt()>0)cout<<"leptonic Top "<<deltaR(topJets_best+wlep_best,GenInfo.get_topLep())<<endl;
